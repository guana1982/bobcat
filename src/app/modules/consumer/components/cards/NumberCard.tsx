...
const NumberCard_ = (props: NumberCardProps) => {
  ...
  const savedBottle = parseInt(!isLogged ? savedBottleMachine : savedBottleConsumer);

  let messageBottleSave = "";
  if (savedBottle >= 0 && savedBottle <= 10) {
    messageBottleSave = "c_you_make_impact";
  } else if (savedBottle > 10 && savedBottle <= 30) {
    messageBottleSave = "c_good_job";
  } else if (savedBottle > 30 && savedBottle <= 75) {
    messageBottleSave = "c_keep_it_going";
  } else if (savedBottle > 75 && savedBottle <= 200) {
    messageBottleSave = "c_keep_drinking";
  } else if (savedBottle > 200) {
    messageBottleSave = "c_amazing";
  }
  ...
  return (
    <div className={className}>
      ...
      <div id="text-wrap">
        {!isLogged ?
          <h2>{__("c_make_impact")}</h2> :
          <h2>{__(messageBottleSave)}</h2>
        }
        {!isLogged ?
